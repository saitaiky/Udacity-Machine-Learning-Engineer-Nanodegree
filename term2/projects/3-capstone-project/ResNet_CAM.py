...
def ResNet_CAM(img_path, model, all_amp_layer_weights):
    ...
    mat_for_mult = scipy.ndimage.zoom(last_conv_output, (8, 8, 1), order=1) # dim: 224 x 224 x 2048
    ...
    final_output = np.dot(mat_for_mult.reshape((224*224, 32)), amp_layer_weights).reshape(224,224) # dim: 224 x 224
...
def plot_ResNet_CAM(img_path, ax, model, all_amp_layer_weights):
    ...
    imagenet_classes_dict = ['crazy', 'inclusion', 'patches', 'pitted surface', 'rolled-in scale', 'scratches']
