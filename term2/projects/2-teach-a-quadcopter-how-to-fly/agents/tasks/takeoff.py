...
class Task():
    ...
    def __init__(self, init_pose=None, init_velocities=None,
        ...
        self.max_duration = runtime # secs
    ...
    def get_reward(self):
        ...
        # penalty for euler angles, we want the takeoff to be stable and to limit the angular movement of the agent.
        penalty += abs(self.sim.pose[3:6]).sum()

        # prioritizing the z-axis in the penalty term, as the agent needs only vertical force to take off, is a very good decision.
        ...
        # extra reward for flying near the target - so that the agent is prompted to learn the optimal policy.
        ...
        return reward - penalty*.0002


    def step(self, rotor_speeds, timestamp):
        ...
        for _ in range(self.action_repeat):
            ...
            # Bonus Penalizing: the agent with the negative rewards can be considered when the agent crashes.
            if  timestamp > self.max_duration:  # agent has run out of time
                reward -= 10.0  # extra penalty
                done = True

            # Bonus reward: Z axis is being prioritized.
